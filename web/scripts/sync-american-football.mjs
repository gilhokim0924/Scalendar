...
const DELAY_MS = 2500;
const SPORT_KEY = 'american_football';
const SPORT_NAME = 'American Football';
...
const SEASON = getCurrentAmericanFootballSeason();
...
async function main() {
  ...
  console.log(`Starting american football sync for ${SEASON}`);
  ...
  console.log(`American football sync complete: ${allTeams.length} teams, ${allEvents.length} events, ${standings.length} standings rows`);
}

main().catch((error) => {
  console.error('American football sync failed:', error);
  process.exitCode = 1;
});
