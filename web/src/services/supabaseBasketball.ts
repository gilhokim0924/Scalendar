...
const BASKETBALL_SPORT_KEY = 'basketball';
...
const CURRENT_SEASON = getCurrentBasketballSeason();
...
async function getBasketballCompetition(leagueId: string) {
  const { data, error } = await supabase
    .from('competitions')
    .select('id, name, sports!inner(key)')
    .eq('sports.key', BASKETBALL_SPORT_KEY)
...
export async function fetchBasketballTeams(leagueId: string) {
  const { data, error } = await supabase
    .from('teams')
...
export async function fetchBasketballEvents(leagueId: string) {
  const competition = await getBasketballCompetition(leagueId);
...
export async function fetchBasketballStandings(leagueId: string) {
  const competition = await getBasketballCompetition(leagueId);
  if (!competition) return [];
