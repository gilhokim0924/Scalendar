...
const BASEBALL_SPORT_KEY = 'baseball';

function getCurrentBaseballSeason(now = new Date()) {
  return String(now.getUTCFullYear());
}

const CURRENT_SEASON = getCurrentBaseballSeason();
...
async function getBaseballCompetition(leagueId: string) {
  const { data, error } = await supabase
    .from('competitions')
    .select('id, name, sports!inner(key)')
    .eq('sports.key', BASEBALL_SPORT_KEY)
...
export async function fetchBaseballTeams(leagueId: string) {
  const { data, error } = await supabase
    .from('teams')
...
export async function fetchBaseballEvents(leagueId: string) {
  const competition = await getBaseballCompetition(leagueId);
...
export async function fetchBaseballStandings(leagueId: string) {
  const competition = await getBaseballCompetition(leagueId);
  if (!competition) return [];
