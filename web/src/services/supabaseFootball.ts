...
async function getFootballCompetition(leagueId: string) {
  const { data, error } = await supabase
    .from('competitions')
    .select('id, name, sports!inner(key)')
    .eq('sports.key', FOOTBALL_SPORT_KEY)
...
export async function fetchEvents(leagueId: string, rounds?: number[]) {
  const competition = await getFootballCompetition(leagueId);
  ...
  if (rounds?.length) {
    query = query.in('round', rounds.map(String));
  }
